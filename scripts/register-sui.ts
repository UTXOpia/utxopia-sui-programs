...
// Native SUI metadata has migrated away from the legacy CoinMetadata object on current testnet,
// so this calls the admin-gated token_registry::register_token_with_decimals<T>. Amounts are in
// MIST (9 decimals).
//
...
console.log(`Registering ${COIN_TYPE} with decimals=${DECIMALS}`);

const result = spawnSync("sui", [
  "client", "call",
  "--package", packageId,
  "--module", "token_registry",
  "--function", "register_token_with_decimals",
  "--type-args", COIN_TYPE,
  "--args", adminCap.objectId, pool.objectId, registry.objectId, DECIMALS,
